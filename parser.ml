...
let extract_ident s =
  let rec extract_aux s =
    ...
    | _ -> ([], s)
  in
  match s with c :: _ when is_alpha c -> extract_aux s | _ -> ([], s)
...
let rec lexer s =
  ...
  | x :: _ when is_digit x ->
      ...
      if x = [] then raise LexingError
      else
        Num (int_of_string (String.concat "" (List.map (String.make 1) x)))
        :: lexer q
  | _ ->
      let x, q = extract_ident s in
      if x = [] then raise LexingError
      else
        let x = String.concat "" (List.map (String.make 1) x) in
        let xl = String.lowercase_ascii x in
        (if List.mem xl keywords then Keyword xl else Ident x) :: lexer q

let tokenize s = lexer (List.of_seq (String.to_seq s))
...
let rec parse_formula2 tl =
  ...
  | LPar :: q -> (
      ...
      match tl' with RPar :: q' -> (f, q') | _ -> raise ParsingError)
  | TNot :: tl' ->
  ...
  | Ident x :: LPar :: q -> (
      let term_list, tl' = parse_term_list q in
      match tl' with
      | RPar :: q' -> (Formula.Rel (x, term_list), q')
      | _ -> raise ParsingError)
...
let parse_command tl =
  let c, remaining =
    ...
    | Keyword "define" :: Ident rel :: Num n :: q ->
        let f, q = parse_formula q in
        (Command.Define (rel, n, f), q)
    | Keyword "unroll" :: q -> (Command.Unroll, q)
    | Keyword "axiom" :: q -> (Command.ApplyRule Deduction.Axiom, q)
    | Keyword "assume" :: q -> (Command.ApplyRule Deduction.Assume, q)
    ...
    | Keyword "elim" :: TOr :: q -> (
        ...
        | _ -> raise ParsingError)
    | Keyword "intro" :: TAnd :: q -> (Command.ApplyRule Deduction.IntroAnd, q)
    ...
    | Keyword "intro" :: TNot :: q -> (Command.ApplyRule Deduction.IntroNot, q)
    | Keyword "elim" :: TNot :: q ->
        let f, q = parse_formula q in
    ...
    | Keyword "classical" :: q -> (Command.ApplyRule Deduction.Classical, q)
    | Keyword "peirce" :: q -> (Command.ApplyRule Deduction.Peirce, q)
    | Keyword "auto" :: q -> (Command.Auto, q)
  ...
  if remaining <> [] then
    raise
      (ExtraTokenError (String.concat ";" (List.map string_of_token remaining)))
  else c
